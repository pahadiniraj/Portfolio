...
const AdminProject: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (isConfirm) {
      try {
        toast.info("Deleting project..."); // Feedback during deletion
        const response = await DeleteProject(id).unwrap(); // Using unwrap to handle success and error
        if (response.success) {
          ...
          : projects.map((value: Project) => (
              ...
              >
                ...
                <div className="md:w-[400px] flex flex-col gap-2 justify-center">
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-3 gap-2">
                    {value.technologies.map((tech: string, index: number) => (
                      <p
                ...
                <div className="gap-8 flex md:flex-col mt-2 md:mt-0 md:justify-center items-center p-2 md:w-1/6">
                  <button
                    ...
                    disabled={deleteLoading} // Disable button during delete
                  >
